...
const SignIn = ({toggle}) => {
  ...
  return (
      <div className="bg-neutral-800 w-80 flex flex-col h-max rounded-lg  px-4 ">
           ...
           <InputBox onChange={(e)=>setUserName(e.target.value)} title={"Email"} holder={"leoj@example.com"}></InputBox>
           ...
           <Button buttonlabel={"Sign In"} onClick={async()=>{
            try{
              const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/user/sigin`,{
                    username,password })
